...
export const useSessionStore = defineStore("session", {
  ...
  actions: {
    applySession(payload, feedback = "") {
      ...
      if (this.user?.id) {
        stateContinuity.open({ id: this.user.id, preferences: this.user.preferences || {} }).control.sync().catch(() => {});
      }
    ...
    restoreOfflineSession(payload, feedback = "当前离线，已显示最近一次同步内容") {
      ...
      this.status = "ready";
      this.apiStatus = "offline";
      const scope = stateContinuity.open({ id: restoredUser.id, preferences: restoredUser.preferences || {} });
