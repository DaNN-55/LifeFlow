...
> [!NOTE]
> **Context**: Standalone `FretFlow/` is being merged into the current LifeFlow app.
...
将独立项目 `/Users/dan/Programs/LifeFlow/FretFlow` 中与吉他练习相关的全部能力，迁移到当前主项目 `/Users/dan/Programs/LifeFlow` 的 `FretFlow` 一级 tab（`/fretflow`）内，保留当前 LifeFlow 的导航结构与视觉语言，不再继续依赖旧项目里的 legacy runtime。
...
- 一级导航：`/fretflow`
- 页面文件：`src/views/FretFlowView.vue`
...
- `src/stores/fretflow.js`
- `src/components/fretflow/FretFlowPracticePanel.vue`
- `src/components/fretflow/FretTrainingControls.vue`
- `src/components/fretflow/Fretboard.vue`
- `src/components/fretflow/CagedControls.vue`
- `src/components/fretflow/CircleOfFifthsPanel.vue`
- `src/components/fretflow/MetronomePanel.vue`
- `src/components/fretflow/FretFlowHeaderActions.vue`
- `src/composables/useTrainingAudio.js`
- `src/composables/useMetronome.js`
- `src/utils/fretflow/music-theory.js`
- `src/utils/fretflow/fretboard.js`
- `src/utils/fretflow/phrase-files.js`

### 修改

- `src/views/FretFlowView.vue`
- `src/styles/base.css`
- `src/stores/account.js` 或对应 preferences merge 入口
- `backend/src/app.js`
...
- 2026-03-25: Initial migration plan created after reviewing standalone `FretFlow` live app, legacy runtime, archived componentized attempt, and current LifeFlow integration point.
